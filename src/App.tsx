...
function App() {
  ...
  const [moves, setMoves] = useState<{fromCell: {ID: string, pieceID: string | null}, toCell: {ID: string, pieceID: string | null}}[]>([])
  ...
  const onCellClick = (cellID: string):void => {
    ...
    setMoves(
      moves.concat(
        [{fromCell: rememberedCell, toCell: currentCell}]
      )
    )
  }
  ...
  return (
    <div className="App">
      ...
      <ControlPanel cells={cells} isWhitePlayerCurrent={isWhitePlayerCurrent} moves={moves} pieces={PIECES} onResetGame={resetGame} />
    </div>
  );
}
